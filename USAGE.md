...
`landing_cro_audit_run`, `rfm_segmentation`.
...
retention ROI) → `rfm_segmenter` (who to win back, who to nurture) → `cohort_ltv` +
`unit_economics` to size the impact.
...
RU/CIS. У тебя подключён MCP-коннектор NECTARIN с 64 инструментами под полный
...
3. Подбирай инструмент под услугу клиента:
   • Инфлюенсеры → influencer_planner · OLV/видео → reach_frequency, frequency_cap_optimizer,
     ...
     Удержание/CRM → churn_predictor, rfm_segmenter, cohort_ltv · Конверсия/CRO → landing_cro_audit, funnel_model ·
