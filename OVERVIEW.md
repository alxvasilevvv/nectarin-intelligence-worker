...
**AI-маркетинг-директор для рынка RU/CIS прямо в Claude.** MCP-коннектор с **64
детерминированными инструментами** и **41 готовым сценарием (prompts)**, которые
...
- **41 готовый сценарий.** Нажал prompt — Claude сам вызывает нужные инструменты в
...
- **Удержание:** `churn_predictor` → `rfm_segmenter` → `cohort_ltv` → `unit_economics`.
...
*NECTARIN Intelligence · 64 инструмента · 41 сценарий · Cloudflare Workers (edge,
